...
while ($row_1 = mysqli_fetch_array($qry_1, MYSQLI_ASSOC)) {
    ...
    // untuk retail
    $sql_3 = "SELECT tb_retail_pemesanan.waktu, SUM(tb_retail_transaksi.sub_total) AS pemasukan FROM tb_retail_pemesanan LEFT JOIN tb_retail_transaksi ON tb_retail_pemesanan.kd_transaksi = tb_retail_transaksi.kd_transaksi WHERE MONTH ( waktu ) = '$bulan' GROUP BY waktu";
    $qry_3 = mysqli_query($mysqli, $sql_3);
    while ($row_3 = mysqli_fetch_array($qry_3)) {
        $data[$bulan][] = $row_3['pemasukan'];
    }
}
