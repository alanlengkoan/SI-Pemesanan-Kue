...
if(isset($_GET['Kode'])) {
	...
	$mySql  = "SELECT pemesanan.*, pelanggan.nm_pelanggan, provinsi.* FROM pemesanan LEFT JOIN pelanggan ON pemesanan.kd_pelanggan= pelanggan.kd_pelanggan LEFT JOIN provinsi ON pemesanan.kd_provinsi=provinsi.kd_provinsi WHERE pemesanan.kd_pelanggan='$KodePelanggan' AND pemesanan.no_pemesanan ='$Kode'";
...
<table width="761" border="0" cellpadding="2" cellspacing="0">
  ...
  <?php 
	...
	$tampilQry = mysqli_query($mysqli, $tampilSql) or die ("Gagal SQL".mysqli_error());
	$no	= 0; 
  ...
  <?php } 
  	// Menghitung
		...
		$totalBiayaKirim = $myData['biaya_kirim'];
